...
data class Advice(
    ...
    val anythingNeeded: Boolean get() = rain != Need.NO || warm != Need.NO
    val definite: Boolean get() = rain == Need.YES || warm == Need.YES

    /** 0 = nothing to worry about, 3 = take everything. */
    val severity: Int
        get() {
            val r = when (rain) { Need.NO -> 0; Need.MAYBE -> 1; Need.YES -> 2 }
            val w = when (warm) { Need.NO -> 0; Need.MAYBE -> 1; Need.YES -> 2 }
...
object Jacket {
    ...
    fun forRides(rides: List<RideAssessment>, s: Settings): Advice {
        if (rides.isEmpty()) return Advice(Need.NO, Need.NO, emptyList(), rides, false)
        ...
        val warm = when {
            coldest == null -> Need.NO
            coldest <= s.warmJacketTemp -> Need.YES
            coldest <= s.warmJacketTemp + 3.0 -> Need.MAYBE
            else -> Need.NO
        }

        val extras = buildList {
            if (coldest != null) {
                if (coldest <= s.warmJacketTemp - 5.0) add(Extra.GLOVES)
        ...
        return Advice(rain, warm, extras, rides, withConditions.isNotEmpty())
    }
}
